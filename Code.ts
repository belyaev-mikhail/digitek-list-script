...
function updateSummary() {
  ...
  var others = sheet.getSheets().filter(function (it) { return it.getName() !== summary.getName() })

  var formula = "SORT({" + others.map(function(it){ return "'" + it.getName() + "'!A2:D44" }).join("; ") + "}; 2; TRUE)"
